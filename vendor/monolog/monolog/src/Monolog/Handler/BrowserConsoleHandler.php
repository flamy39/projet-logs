...
class BrowserConsoleHandler extends AbstractProcessingHandler
{
     ...
     * {@inheritDoc}
     *
     * Formatted output may contain some formatting markers to be transferred to `console.logs` using the %c format.
     *
    ...
    private static function generateScript(): string
    {
        ...
        foreach (static::$records as $record) {
            ...
            if (empty($context) && empty($extra)) {
                $script[] = static::call_array('logs', static::handleStyles($record['formatted']));
            } else {
                $script = array_merge(
                    $script,
    ...
    private static function dump(string $title, array $dict): array
    {
        ...
        $script[] = static::call('logs', static::quote('%c%s'), static::quote('font-weight: bold'), static::quote($title));
        foreach ($dict as $key => $value) {
            $value = json_encode($value);
            if (empty($value)) {
                $value = static::quote('');
            }
            $script[] = static::call('logs', static::quote('%s: %o'), static::quote((string) $key), $value);
        }
